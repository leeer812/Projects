...
/**
 * This class defines and creates a user interface using the
 * {@link ApplicationTemplate} class. The interface created here at the
 * framework-level does not instantiate an actual workspace (which must be done
 * by any application using this framework).
 *
...
public class UITemplate implements UIComponent
{
    ...
    /**
     * Creates the minimal user interface to be used by a Vilij application. It
     * uses the window height and width properties and creates a toolbar with
    ...
    /**
     * Initialization is not provided at the template-level, and must be
    ...
    /**
     * This action is not provided at the template-level, since there is nothing
     * to clear out. Implementation must be done in a child class at the
